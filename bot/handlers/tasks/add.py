...
@dp.callback_query_handler(
    lambda c: re.match("category_[A-z0-9 _]*", c.data), state=FSMAddTask.category
)
...
async def add_timeshift(
    ...
    await bot.send_message(
        callback_query.from_user.id, "Укажите время на выполнение задачи"
    )
    await FSMAddTask.next()
    await bot.delete_message(
        callback_query.from_user.id, callback_query.message.message_id
...
async def add_links(message: types.Message, state: FSMContext) -> None:
    async with state.proxy() as data:
        ...
        await TaskService.create_task(
            TaskCreateScheme(**data, user_id=message.from_user.id)
        )
    await state.finish()
    await bot.delete_message(message.from_user.id, message.message_id)
