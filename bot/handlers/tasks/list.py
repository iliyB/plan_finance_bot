...
@dp.callback_query_handler(
    lambda c: c.data == CommandEnum.ALL_PLANED_TASKS.value, state=None
)
async def all_planed_task(callback_query: types.CallbackQuery) -> None:
    ...
    if tasks:
        await bot.send_message(
            callback_query.from_user.id,
            "Задачи",
            reply_markup=create_retrieve_task_keyboard(tasks),
        )
        await FSMTask.tasks.set()
    else:
        await bot.send_message(
            callback_query.from_user.id, "У вас нет запланированных задач"
        )


@dp.callback_query_handler(
    lambda c: c.data == CommandEnum.TODAY_TASKS.value, state=None
)
async def task_on_today(callback_query: types.CallbackQuery) -> None:
    await bot.delete_message(
        callback_query.from_user.id, callback_query.message.message_id
    )
    tasks = await TaskService.all_planed_tasks_for_user_on_today(
        callback_query.from_user.id
    )
    if tasks:
        await bot.send_message(
...
@dp.callback_query_handler(
    lambda c: c.data == CommandEnum.WEEKLY_TASKS.value, state=None
)
async def task_on_week(callback_query: types.CallbackQuery) -> None:
    await bot.delete_message(
        callback_query.from_user.id, callback_query.message.message_id
    )
    tasks = await TaskService.all_planed_tasks_for_user_on_week(
        callback_query.from_user.id
    )
    if tasks:
        await bot.send_message(
...
@dp.callback_query_handler(
    lambda c: c.data == CommandEnum.PERIOD_TIME_TASKS.value, state=None
)
async def task_in_date_period(
...
@dp.callback_query_handler(
    simple_cal_callback.filter(), state=FSMDatePeriod.second_date
)
async def task_second_date(
    callback_query: types.CallbackQuery, state: FSMContext, callback_data: Dict
) -> None:
    selected, date = await SimpleCalendar().process_selection(
        callback_query, callback_data
    ...
    async with state.proxy() as data:
        ...
        if first_date > date:
            await bot.send_message(
                callback_query.from_user.id, "Первая дата больше второй"
            )
        else:
            tasks = await TaskService.all_planed_tasks_for_user_on_period(
                callback_query.from_user.id, first_date, date
            )
            if tasks:
                await bot.send_message(
                    callback_query.from_user.id,
